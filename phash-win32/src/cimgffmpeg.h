 ...
 * 
 *  NOTE: opens and closes the file within function. No need to do so outside function call.
 *
...
/*
 	...
 	of the stream.  NOTE: Will open file at beginning and close file at end, no need to call vf_info_close()
    unless finished reading frames  before end of call.
 	@param st_info - ptr to struct for state information.
 	@param pFrameList CImgList<>* - an empty CImgList<> in which to append newly read frames 
 	@return int number of frames read or 0 if end of stream reached,  -1 unable to read from stream
*/
__declspec(dllexport)
