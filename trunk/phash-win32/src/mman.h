...
*  @param prot - int protection flags (flags passed to CreateFileMapping in windows)
*                available flags: PAGE_READONLY, PAGE_READWRITE, PAGE_WRITECOPY
*  @param flags - int extra flags (flags passed to MapView of File in windows)
*                available flags: FILE_MAP_ALL_ACCESS, FILE_MAP_COPY, FILE_MAP_READ, FILE_MAP_FILE_MAP_WRITE
...
*  (errno is set to EACCESS if file is locked against access, EBADF if file is readonly or handle is invalid,
*   ENOSPC if not enough space left on the device)
*/
 ...
 * @return int value - 0 for success, -1 for error (use GetLastError() to get extended information)
*/
