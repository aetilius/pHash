...
__attribute__((__deprecated__)) void ph_mvp_init(MVPFile *m)
{
	if(!m) return;
...
__attribute__((__deprecated__)) MVPRetCode ph_query_mvptree(MVPFile *m, DP *query, int knearest, float radius, float threshold,
...
__attribute__((__deprecated__)) MVPRetCode ph_save_mvptree(MVPFile *m, DP **points, int nbpoints){
...
__attribute__((__deprecated__)) MVPRetCode ph_add_mvptree(MVPFile *m, DP **points, int nbpoints, int &nbsaved){
    nbsaved = 0;
    /* open main file */
    char mainfile[256];
    snprintf(mainfile, sizeof(mainfile),"%s.mvp", m->filename);
